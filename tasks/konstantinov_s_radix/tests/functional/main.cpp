...
class KonstantinovSRadixTests : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 ...
 protected:
  void SetUp() override {
    ...
    TestType fileparam =std::get<static_cast<std::size_t>(ppc::util::GTestParamIndex::kTestParams)>(GetParam()) + ".txt";
...
const auto kTestTasksList = std::tuple_cat(ppc::util::AddFuncTask<KonstantinovSRadixMPI, InType>(
                                               kTestParam, PPC_SETTINGS_konstantinov_s_radix),
                                           ppc::util::AddFuncTask<KonstantinovSRadixSEQ, InType>(
                                               kTestParam, PPC_SETTINGS_konstantinov_s_radix));
